...
func TestAnalyseFrequency(t *testing.T) {
	...
	if !reflect.DeepEqual(expectedFrequency, frequency) {
		t.Errorf("Expected %d, but got %d", expectedFrequency, frequency)
	}

	if !reflect.DeepEqual(expectedUniques, uniques) {
		t.Errorf("Expected %d, but got %d", expectedUniques, uniques)
	}
}
